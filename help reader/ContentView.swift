...
struct ContentView: View {
    ...
    //chatgpt experiment
   // @State private var selectedChatOption: String?
    ...
    var body: some View {
        NavigationStack {
            BookView(text: bookText, pageSize: 400)
                .toolbar {
                    ToolbarItem(placement: .navigationBarTrailing) {
                       Button(action: saveBook) {
                           Image(systemName: "square.and.arrow.down")
                       }
                   //    .popover(isPresented: $selectedChatOption, content: {
                     //      VStack {
                           //    ForEach(chatOptions, id: \.self) { option in
                           //        Button(action: {
                            //           handleChatOptionSelected(option: option)
                              //     }) {
                               //        Text(option)
                              //             .padding()
                                 //  }
    ...
    func saveBook() {
        // Save book logic
        // Add your code to save the book here
    }
}
